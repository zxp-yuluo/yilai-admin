    ...
    <el-form label-width="120px" style="margin-top: 10px;" :label-position="labelPosition">
      <el-collapse v-model="activeNames">
        <el-collapse-item title="商品基本信息" name="productBaseInfo">
          ...
          <el-form-item label="分类">
            <el-cascader style="width: 100%;" :props="categoryProps" v-model="categoryIdList" @change="categoryChange" />
          </el-form-item>
          <el-form-item label="品牌">
          ...
          <el-form-item label="轮播图">
            <el-upload v-model:file-list="fileList" action="http://localhost:1992/admin/system/fileUpload"
          ...
          <el-form-item label="商品SKU">
            <el-table :data="product.productSkuList" border style="width: 100%">
              ...
              <el-table-column label="图片" #default="scope" width="80">
                <el-upload class="avatar-uploader" action="http://localhost:1992/admin/system/fileUpload"
        ...
        <el-collapse-item title="商品详情信息" name="productDetails">
          <el-form-item label="详情图片">
            <el-upload v-model:file-list="detailsFileList" action="http://localhost:1992/admin/system/fileUpload"
...
import { getAllBrandList } from '../../../api/brand/brand'   // FindAllBrand
import { getCategoryList } from '../../../api/category/category'   // FindCategoryByParentId
import { GetProductPageList, SaveProduct, UpdateProductById, GetProductById, DeleteProductById, UpdateProductStatus, UpdateProductAuditStatus } from '../../../api/product/product'
import { FindBrandByCategoryId } from '../../../api/categoryBrand/categoryBrand'
import { FindAllProductUnit } from '../../../api/product/productUnit.js'
import { FindAllProductSpec } from '../../../api/product/productSpce'
...
// 钩子函数
onMounted(async () => {
  const { data } = await getAllBrandList()
...
const headers = {
  token: useApp().token     // 从pinia中获取token，在进行文件上传的时候将token设置到请求头中
}
